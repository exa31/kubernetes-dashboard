  ...
  >
    ...
    >
      <div class="flex items-center gap-3">
        <div
          class="w-9 h-9 rounded-xl bg-emerald-500/15 border border-emerald-500/30 flex items-center justify-center text-emerald-400 shrink-0"
        >
        ...
        <div>
          <h3 class="text-sm font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
            ...
            <span
              class="px-2 py-0.5 rounded text-[10px] font-mono bg-emerald-500/20 text-emerald-300 border border-emerald-500/30"
            >
              {{ serviceName }}
            </span>
          </h3>
          <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            Active backend Pod IPs and health check state in namespace
            <strong class="font-mono text-slate-600 dark:text-slate-300">{{ namespace }}</strong>
          </p>
        </div>
      </div>
    ...
    <div class="p-6 space-y-6 max-h-[75vh] overflow-y-auto">
      ...
      <div
        v-if="error"
        class="p-4 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-400 text-xs flex items-center gap-2"
      >
        ...
        <div class="space-y-2">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5">
            ...
            >
              ...
              <span
                class="px-2.5 py-1 rounded-lg text-xs font-mono font-bold bg-sky-500/10 text-sky-400 border border-sky-500/30"
              >
                :{{ p.port }}
              </span>
            </div>
            <div
              v-if="endpoints.ports.length === 0"
              class="col-span-full text-xs text-slate-500 italic p-3"
            >
        ...
        <div class="space-y-2">
          <div class="flex items-center justify-between">
            ...
            <span class="text-[11px] text-slate-400 font-mono">
              Ready: {{ endpoints.targets.filter((t) => t.ready).length }} /
              {{ endpoints.targets.length }}
            </span>
          </div>

          <div
            v-if="endpoints.targets.length === 0"
            class="p-8 text-center rounded-2xl bg-amber-500/5 border border-amber-500/20 text-amber-300"
          >
            ...
            <p class="text-[11px] text-amber-400/80 mt-1 max-w-md mx-auto">
              This Service does not currently point to any running Pod. Check if the Service's
              selector matches your deployment labels.
            </p>
          </div>
          ...
          <div v-else class="space-y-2">
            ...
            >
              <div class="flex items-center gap-3">
                ...
                <div>
                  <div
                    class="text-xs font-bold text-slate-900 dark:text-slate-100 font-mono flex items-center gap-2"
                  >
                    <span>{{ target.pod_name || 'Direct IP Target' }}</span>
                  </div>
                  <div class="flex items-center gap-3 text-[11px] text-slate-400 font-mono mt-0.5">
                    <span
                      >IP: <strong class="text-slate-300">{{ target.ip }}</strong></span
                    >
                    <span>&bull;</span>
                    <span
                      >Node:
                      <strong class="text-slate-300">{{
                        target.node_name || 'unassigned'
                      }}</strong></span
                    >
                  </div>
                </div>
    ...
    <!-- Footer -->
    <div
      class="px-6 py-3 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/90 flex justify-end"
    >
      <Button
        label="Close"
        size="small"
